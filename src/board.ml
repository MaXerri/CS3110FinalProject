...
let parse_boolean_lists above at below =
  ...
  let rec pbl_helper (above : bool list) (at : bool list) (below : bool list) =
    if
      let l_at = List.length at in
      l_at = List.length above && l_at = List.length below
    then
      match (above, at, below) with
      (*Functional case: 9 integers used to generate a single cell*)
      | h0 :: h1 :: h2 :: t_abv, i0 :: i1 :: i2 :: t_at, j0 :: j1 :: j2 :: t_bel
        ->
          let c =
            if i1 then Cell.generate (-1)
            else
              Cell.generate
                (bool_list_to_int [ h0; h1; h2; i0; i2; j0; j1; j2 ])
          in
          c
          :: pbl_helper (h1 :: h2 :: t_abv) (i1 :: i2 :: t_at)
               (j1 :: j2 :: t_bel)
      (*End Case: Last two elements in each row reached, return the empty list*)
      | [ _; _ ], [ _; _ ], [ _; _ ] -> []
      (*Unexplained Case: Should never occur if implementation is correct*)
      | _, _, _ ->
          raise
            (Failure
               {|Improper parsing in Board.parse_boolean_lists "Internal helper 2"|})
    else failwith {|[parse_boolean_lists] fed lists of unequal size|}
  in
  (*Execution*)
