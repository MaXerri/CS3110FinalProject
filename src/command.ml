...
(* let int_to_char n = let rec int_to_char_aux num (acc_chars : char list) = let
   remainder = num mod 26 in let quotient = num / 26 in if quotient = 0 then
   (Char.chr (remainder + 65)) :: acc_chars else int_to_char_aux (quotient - 1)
   ((Char.chr (remainder + 65)) :: acc_chars) in let char_list = int_to_char_aux
   n [] in char_list |> List.map (fun c -> String.make 1 c) |> String.concat
   "" *)
...
let is_alphanumeric str =
  ...
  let rec loop i =
    if i < String.length str then
      if not (String.contains valid_chars str.[i]) then false else loop (i + 1)
    else true
  in
  loop 0
...
let check_malformed str_list =
  ...
  | h :: t ->
      if
        h <> "quit" && h <> "clear" && h <> "flag" && h <> "restart"
        && h <> "help" && h <> "rules"
      then true
    ...
    | h :: t ->
        if h = "clear" then
          if List.length t = 2 then
            ...
            let row = try int_of_string x with Failure _ -> char_to_int x in
            let col = try int_of_string y with Failure _ -> char_to_int y in
            Clear (row, col)
